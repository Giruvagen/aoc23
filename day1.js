exports.day1function = (data) => {
    let numbers = data.map(x => {
        let nums = [];
        const chars = x.split('');
        console.log(chars);
        chars.map((e) => {
            if (matcher(e).length > 0) {
                nums.push(e)
            }
        });
        if (nums.length > 1) {
            return parseInt(nums[0].toString() + nums[nums.length - 1].toString())
        } else {
            return parseInt(nums[0].toString());
        }
    })

    const sum = numbers.reduce((partialSum, a) => partialSum + parseInt(a), 0);
    console.log(numbers)
    return sum;
}
